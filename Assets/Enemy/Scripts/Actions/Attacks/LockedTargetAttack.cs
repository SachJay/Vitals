...
public class SavedTargetFire : EnemyAttack
{
    ...
    {
        if (savedTarget == null)
            savedTarget = player.transform.position;
        ...
        for (int j = 0; j < numberOfWaves; j++) 
        {
            ...
            {
                Vector3 targetDir = savedTarget - transform.position;// + player.PlayerMovement.GetVelocity() * predictiveAmount;
